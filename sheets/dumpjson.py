    ...
    try:
        ...
        for row in date_and_ids[-7:]:
            ...
            for s in songs:
                ...
                dbvalues = [v.replace("'", "''") for v in cols]
                dbvalues = [f"'{v}'" for v in dbvalues]
                dbvalues = [v.replace("'XX'", 'NULL') for v in dbvalues]
                insert_rows.append(
                    f"insert into setlists ({','.join(dbfields)}) values({','.join(dbvalues)})"
                )
        print('\n'.join(insert_rows))
